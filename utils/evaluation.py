...
def get_top_k_accuracies(labels, scores, k):
    accs = list()
    for i in range(1, k + 1):
        acc = sklearn.metrics.top_k_accuracy_score(labels, scores, k=i)
        accs.append((i, acc))
    return pd.DataFrame(accs, columns=["k", "acc"])

def get_evaluations(y_true, y_pred, model_outputs, model_name: str) -> pd.DataFrame: # macro/micro-f1; Cohen's kappa; Matthew’s correlation coefficient; h@1,3,6,9
